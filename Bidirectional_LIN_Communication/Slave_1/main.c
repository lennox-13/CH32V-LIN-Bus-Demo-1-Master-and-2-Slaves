 ...
 * Universal LIN slave
 *
 * - Reads local button on PC1
 * - Sends button state on STATUS_ID request
 * - Receives LED command on CMD_ID frame
 * - Drives local LED on PD4
 *
 * To build Slave 1:
 *   STATUS_ID = 0x22
 *   CMD_ID    = 0x24
...
/* ---------------- Configure per slave ---------------- */
#define STATUS_ID             0x22u
#define CMD_ID                0x24u

/* Set LED polarity for your board:
 * 1 = LED active LOW
 * 0 = LED active HIGH
 */
...
#define BTN_PRESSED()         (GPIO_ReadInputDataBit(BTN_PORT, BTN_PIN) == RESET)
...
static void LIN_SlaveSendStatus(void)
{
    uint8_t pid  = LIN_GetPid(STATUS_ID);
    uint8_t data = BTN_PRESSED() ? 1u : 0u;
...
static void LIN_ApplyCommand(uint8_t data)
{
    if (data) {
        LED_ON();
    } else {
...
{
    uint8_t d;
    uint8_t status_pid = LIN_GetPid(STATUS_ID);
    uint8_t cmd_pid    = LIN_GetPid(CMD_ID);
    ...
    if (USART_GetITStatus(USART1, USART_IT_RXNE) != RESET) {
        ...
        case 4:
        {
            ...
            cmd_cs = d;
            exp_cs = LIN_Checksum_Enhanced(cmd_pid, (const uint8_t*)&cmd_data, 1u);

            if (cmd_cs == exp_cs) {
                LIN_ApplyCommand(cmd_data);
            }
